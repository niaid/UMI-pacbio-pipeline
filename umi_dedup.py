"""
This module executes fake umi removal using a network process
"""
import sys
import time
import os
...
def check_gen_dist(sample_tuple, fasta_reads):
    ...
    if hamming_distance(str(fasta_reads[i]), str(fasta_reads[j])) == 0:
        return sample_tuple
...
def build_gen_dist(fasta_reads, n_jobs):
    ...
    ----------
    fasta_reads : list
        list of reads from fasta file. Each read in in SeqIO format
    ...
    keeps = Parallel(n_jobs=n_jobs, batch_size=256)(delayed(check_gen_dist)(sample_tuple, fasta_reads)\
                              for sample_tuple in tqdm(read_tuple_list))
...
def make_edge(sample_tuple, umi_count_dict):
    ...
    criteria = max(int(umi_count_dict[first_read][1]), int(umi_count_dict[second_read][1])) >= \
                (2**(1+100*(k)))*min(int(umi_count_dict[first_read][1]), int(umi_count_dict[second_read][1])) - 1
...
def make_umi_graph(component, umi_count_dict, n_jobs):
    ...
    umi_edges = Parallel(n_jobs=n_jobs)(delayed(make_edge)(sample_tuple, umi_count_dict) for sample_tuple in umi_tuple_list)
...
def make_merge_list(umi_graph, umi_count_dict, merge_dict):
    ...
    while len(umi_graph_copy.nodes) > 1:
        umi_node_counts = [(x, umi_count_dict[x][1])\
                           for x in umi_count_dict if x in set(umi_graph_copy.nodes)]
...
def make_header_dict(merge_dict, fasta_file_dict, umi_count_dict):
    ...
    for merge_key in merge_dict:
        ...
        new_count = sum([int(umi_count_dict[x][1])\
                       for x in [merge_key] + merge_dict[merge_key]])
        new_header = "_".join(old_header.split('_')[:-1])\
            + '_' + str(new_count) + "_Merged"
        for item in [merge_key] + merge_dict[merge_key]:
            umi_dict[strip_umi(fasta_file_dict[item].id)] = "_".join(\
                new_header.split('_')[-3:-1])
...
def main(project, file_path, n_jobs):
    ...
    """
    fasta_path = project + "final_ccs_reads/mafft/" + file_path + "read_mafft.fasta"
    ...
    header_dict, umi_dict, remove_nw_list = make_header_dict(merge_dict,\
                                             fasta_file_dict, umi_count_dict)
