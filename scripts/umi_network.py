...
if __name__ == "__main__":
    ...
    G_trimmed = networkx.Graph()
    G_trimmed.add_nodes_from(G.nodes)
    for c in G.nodes:
        counts_trimmed.append(counts_dict[c] / 10)
    # for c in nx.connected_components(G):
    #     sorted_umi = sorted(c, key=lambda umi: int(umi.split('_')[1]), reverse=True)
    #     lumi = sorted_umi[0]
    #     if len(sorted_umi) > 1:
    ...
    for u1, u2 in combinations(G_trimmed.nodes, 2):
        ...
        umi2 = u2.split('_')[0]
        if seqlib.qhamming(umi1, umi2, limit=1):
            if int(u1.split('_')[1]) > 5*int(u2.split('_')[1]) or int(u1.split('_')[1]) < 0.2*int(u2.split('_')[1]):
    ...
    # G_all_c = networkx.Graph()
    # for c in nx.connected_components(G_all):
    #     if len(c) == 1:
    #         continue
    #     else:
    #         for umi in c:
    #             G_all_c.add_node(umi)
    #         for umi in c:
    #             for edge in umi.edges:
    #                 G_all_c.add_edge(edge[0], edge[1])

    #G_all_isolates = list(nx.isolates(G_all))
    #G_all.remove_nodes_from(G_all_isolates)
    #G.remove_nodes_from(G_all_isolates)
    #G_trimmed.remove_nodes_from(G_all_isolates)

    counts = []
    counts_trimmed = []
    counts_all = []
    for lumi in G_all.nodes:
        counts.append(counts_dict[lumi] / 10)
        counts_all.append(counts_dict[lumi] / 10)

    for lumi in G_trimmed.nodes:
        counts_trimmed.append(counts_dict[lumi] / 10)

    # print(len(G_all), len(G), len(G_trimmed))
    # print(len(counts_trimmed))

    pos = networkx.spring_layout(G_all)
    ...
    plt.subplot(133)
    nx.draw(G_all, pos=pos, node_size=counts_all, linewidths=2, alpha=0.6, with_labels=False)
