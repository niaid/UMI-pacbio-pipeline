...
def main(project, write_folder):
    ...
    in headers are the sample name + frequency of each sequence and the
    sequence as read.
    Parameters
    ----------
    project : string
    ...
    for sample in files:
        ...
        hap_dict = dict(zip([x[0] for x in total_haps], [f"H{x}" for x in range(1, len(total_haps) + 1)]))
        ...
        sequences.sort()
        counts = [(k, sum(1 for _ in g)) for k, g in itertools.groupby(sequences)]
